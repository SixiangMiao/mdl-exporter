...
class War3Model:
    ...
    def get_parent(obj):
        ...
        ):  # TODO: Check if animated - otherwise, make it a helper
            return obj.parent_bone if obj.parent_bone != "" else None

        if parent.type == "EMPTY" and (
            parent.name.startswith("Bone_")
            or parent.name.startswith("bone_")
            or parent.name.startswith("helper_")
        ):
            return parent.name
    ...
    def get_visibility(self, obj):
        if obj.animation_data is not None:
            curve = War3AnimationCurve.get(
                obj.animation_data, "hide_render", 1, self.sequences
            )
    ...
    def from_scene(self, context, settings, report):
        ...
        if settings.use_selection:
            objs = (
                obj for obj in scene.objects if obj.select_get() and obj.visible_get()
            )
        else:
        ...
        for obj in objs:
            ...
            anim_loc = War3AnimationCurve.get(
                obj.animation_data, "location", 3, self.sequences
            )
            if anim_loc is not None and settings.optimize_animation:
                anim_loc.optimize(settings.optimize_tolerance, self.sequences)

            anim_rot = War3AnimationCurve.get(
                obj.animation_data, "rotation_quaternion", 4, self.sequences
            )

            if anim_rot is None:
                anim_rot = War3AnimationCurve.get(
                    obj.animation_data, "rotation_euler", 3, self.sequences
                )

            if anim_rot is not None and settings.optimize_animation:
                anim_rot.optimize(settings.optimize_tolerance, self.sequences)

            anim_scale = War3AnimationCurve.get(
                obj.animation_data, "scale", 3, self.sequences
            )
            ...
            if len(obj.particle_systems):
                ...
                if getattr(data, "mdl_particle_sys"):
                    ...
                    psys.dimensions = Vector(
                        map(abs, settings.global_matrix @ obj.dimensions)
                    )
            ...
            # Collision Shapes
                ...
                if "Box" in obj.name:
                        ...
                        corners.append(
                            ...
                            @ Vector(
                                abs(
                                    ...
                                    * settings.global_matrix.median_scale
                                )
                                * y
                                for x, y in zip(obj.scale, corner)
                            )
                        )
                    ...
                    collider.verts = [
                        vmin,
                        vmax,
                    ]  # TODO: World space or relative to pivot??
                ...
                mesh = self.prepare_mesh(
                    obj, context, settings.global_matrix @ obj.matrix_world
                )

                # Geoset Animation
                vertexcolor_anim = War3AnimationCurve.get(
                    obj.animation_data, "color", 3, self.sequences
                )
                vertexcolor = None
                ...
                if not any((vertexcolor, vertexcolor_anim)):
                    ...
                    if (
                        ...
                        if node is not None:
                            ...
                            vertexcolor = tuple(
                                getattr(node, attr)[0].default_value[:3]
                            )
                            if hasattr(mat.node_tree, "animation_data"):
                                vertexcolor_anim = War3AnimationCurve.get(
                                    mat.node_tree.animation_data,
                                    'nodes["VertexColor"].%s[0].default_value' % attr,
                                    3,
                                    self.sequences,
                                )
                geoset_anim = None
                ...
                if any((vertexcolor, vertexcolor_anim, visibility)):
                    geoset_anim = War3GeosetAnim(
                        vertexcolor, vertexcolor_anim, visibility
                    )
                    geoset_anim_hash = hash(
                        geoset_anim
                    )  # The hash is a bit complex, so we precompute it
                mesh_geosets = set()
                ...
                if armature is not None:
                    if armature.object is None:
                        report(
                            {"ERROR"},
                            "Armature modifier on %s has no object set!" % obj.name,
                        )
                    else:
                ...
                if (armature is None and parent is None) or is_animated:
                    bone = War3Object(
                        obj.name
                    )  # Object is animated or parent is missing - create a bone for it!
                ...
                for tri in mesh.loop_triangles:
                    ...
                    for vert, loop in zip(tri.vertices, tri.loops):
                        ...
                        uv[1] = (
                            1 - uv[1]
                        )  # For some reason, uv Y coordinates appear flipped. This should fix that.
                    ...
                    geoset.triangles.append(
                        (
                            vertexmap[tri.vertices[0]],
                            vertexmap[tri.vertices[1]],
                            vertexmap[tri.vertices[2]],
                        )
                    )
                ...
                if (
                    obj.name.startswith("SND")
                    or obj.name.startswith("UBR")
                    or obj.name.startswith("FTP")
                    or obj.name.startswith("SPL")
                ):
                    ...
                    for datapath in (
                        ...
                        eventobj.track = War3AnimationCurve.get(
                            obj.animation_data, datapath, 1, self.sequences
                    ...
                    obj.name.startswith("Bone_")
                    or obj.name.startswith("bone_")
                    or obj.name.startswith("helper_")
                ):
                ...
                for b in obj.pose.bones:
                    ...
                    bone.pivot = obj.matrix_world @ Vector(
                        b.bone.head_local
                    )  # Armature space to world space
                    bone.pivot = settings.global_matrix @ Vector(
                        bone.pivot
                    )  # Axis conversion
                    datapath = 'pose.bones["' + b.name + '"].%s'
                    bone.anim_loc = War3AnimationCurve.get(
                        obj.animation_data, datapath % "location", 3, self.sequences
                    )  # get_curves(obj, datapath % 'location', (0, 1, 2))

                    if settings.optimize_animation and bone.anim_loc is not None:
                        bone.anim_loc.optimize(
                            settings.optimize_tolerance, self.sequences
                        )

                    bone.anim_rot = War3AnimationCurve.get(
                        obj.animation_data,
                        datapath % "rotation_quaternion",
                        4,
                        self.sequences,
                    )  # get_curves(obj, datapath % 'rotation_quaternion', (0, 1, 2, 3))
                    ...
                    if bone.anim_loc is not None:
                        ...
                        bone.anim_loc.transform_vec(
                            settings.global_matrix @ m.to_3x3().to_4x4()
                        )
                        self.register_global_sequence(bone.anim_loc)
                ...
                if hasattr(obj.data, "mdl_light"):
                    ...
                    light.intensity_anim = War3AnimationCurve.get(
                        obj.data.animation_data,
                        "mdl_light.intensity",
                        1,
                        self.sequences,
                    ...
                    light.atten_start_anim = War3AnimationCurve.get(
                        obj.data.animation_data,
                        "mdl_light.atten_start",
                        1,
                        self.sequences,
                    ...
                    light.atten_end_anim = War3AnimationCurve.get(
                        obj.data.animation_data,
                        "mdl_light.atten_end",
                        1,
                        self.sequences,
                    ...
                    light.color_anim = War3AnimationCurve.get(
                        obj.data.animation_data, "mdl_light.color", 3, self.sequences
                    )  # get_curve(obj.data, ['mdl_light.color'])
                    self.register_global_sequence(light.color_anim)

                    light.amb_color = light_data.amb_color
                    light.amb_color_anim = War3AnimationCurve.get(
                        obj.data.animation_data,
                        "mdl_light.amb_color",
                        3,
                        self.sequences,
                    ...
                    light.amb_intensity_anim = War3AnimationCurve.get(
                        obj.data.animation_data,
                        "mdl_light.amb_intensity",
                        1,
                        self.sequences,
                ...
                camera.target = camera.pivot + matrix.to_quaternion() @ Vector(
                    (0.0, 0.0, -1.0)
                )  # Target is just a point in front of the camera
        ...
        layers = list(
            itertools.chain.from_iterable(
                [material.layers for material in self.materials]
            )
        )
        ...
        for bone in self.objects["bone"]:
            if not any(
                [
                    ...
                    if bone.name in itertools.chain.from_iterable(g.matrices)
                ]
            ):
        ...
        for geoset in self.geosets:
            ...
            geoset.min_extent, geoset.max_extent = calc_extents(
                [x[0] for x in geoset.vertices]
            )
        ...
        for psys in (
            list(self.objects["particle"])
            + list(self.objects["particle2"])
            + list(self.objects["ribbon"])
        ):
            vertices_all.append(
                tuple(x + y / 2 for x, y in zip(psys.pivot, psys.dimensions))
            )
            vertices_all.append(
                tuple(x - y / 2 for x, y in zip(psys.pivot, psys.dimensions))
            )

        self.geoset_anims = list(
            set(g.geoset_anim for g in self.geosets if g.geoset_anim is not None)
        )

        self.global_extents_min, self.global_extents_max = (
    ...
    def to_scene(self, context, global_matrix, folder):
        ...
        for material_id, material in enumerate(self.materials):
            ...
            material_geoset_ids = [
                i
                for i, geoset in enumerate(self.geosets)
                if geoset.material_id == material_id
            ]
            geoset_anims = set(
                anim
                for anim in self.geoset_anims
                if anim.geoset_id in material_geoset_ids
            )
            ...
            if len(geoset_anims) and material.use_const_color:
                ...
                vertex_color_node.location = Vector(
                    (x_offset - (vertex_color_node.width + 200), 320)
                )
                ...
                if geoset_anim.color is not None:
                    ...
                    vertex_color_node.outputs[0].default_value = Vector(
                        (c[0], c[1], c[2], 1)
                    )
                if geoset_anim.color_anim is not None:
                    geoset_anim.color_anim.to_fcurves(
                        mat.node_tree.nodes["VertexColor"].outputs[0],
                        mat.node_tree,
                        "default_value",
                        'nodes["VertexColor].outputs[0]',
                    )
            ...
            def create_node(index, offset):
                ...
                if texture.is_replaceable:
                    ...
                    if mdl_layer.replaceable_id < 3:
                        ...
                        color_node.outputs[0].default_value = (
                            1.0,
                            0.0,
                            0.0,
                            1.0,
                    ...
                    if mdl_layer.replaceable_id == 1:
                        ...
                        glow_node.location = offset - Vector(
                            (glow_node.width + 100, 180)
                        )

                        color_node.location = offset - Vector(
                            (color_node.width + 100, -50)
                        )

                        mapping_node = nodes.new("ShaderNodeMapping")
                        mapping_node.inputs["Location"].default_value = Vector(
                            (-1, -1, 0)
                        )
                        mapping_node.inputs["Scale"].default_value = Vector((2, 2, 1))
                        mapping_node.location = glow_node.location - Vector(
                            (mapping_node.width + 100, 0)
                        )

                        uv_node.location = mapping_node.location - Vector(
                            (uv_node.width + 100, 0)
                        )
                    ...
                    image_name = os.path.basename(
                        texture.image_path.replace(".blp", ".png")
                    )
                ...
                if source_layer.texture_anim_id is not None:
                    ...
                    if texture_anim.location is not None:
                        texture_anim.location.to_fcurves(
                    ...
                    if (
                        texture_anim.rotation is not None
                    ):  # TODO: Needs to convert from quaternion to euler!
                        texture_anim.rotation.to_fcurves(
                    ...
                    if texture_anim.scale is not None:
                        texture_anim.scale.to_fcurves(
                            mapping_node.inputs["Scale"],
                            mat.node_tree,
                            "default_value",
                            'mapping_node.inputs["Scale"]',
                        )
            ...
            def create_join(top_index, bottom_index, offset):
                ...
                bottom_node = None
                if bottom_index > 0:
                    bottom_node = create_join(
                        bottom_index, bottom_index - 1, offset + Vector((-300, 200))
                    )
                else:
                    bottom_node = create_node(
                        bottom_index, offset + Vector((-300, 200))
                    )
            ...
            last_node = create_join(
                last_index, last_index - 1, Vector((x_offset - 400, 0))
            )
        ...
        node_map = {
            node.object_id: node
            for node in list(self.objects["bone"]) + list(self.objects["helper"])
        }
        skinned_matrices = [
            geoset.matrices for geoset in self.geosets if len(geoset.matrices) > 1
        ]
        skinned_bone_ids = set(
            itertools.chain.from_iterable(
                [b for matrix in skinned_matrices for b in matrix]
            )
        )
        armature_obj = None
        armature = None
        ...
        def animate_bone(node):
            ...
            if node.anim_loc is not None:
                ...
                node.anim_loc.to_fcurves(
                    pose_bone,
                    armature_obj,
                    "location",
                    'pose.bones["%s"].location' % node.name,
                    matrix,
                )
            if node.anim_rot is not None:
                node.anim_rot.transform_rot(global_matrix)
                node.anim_rot.transform_rot(matrix)
                node.anim_rot.to_fcurves(
                    pose_bone,
                    armature_obj,
                    "rotation_quaternion",
                    'pose.bones["%s"].rotation_quaternion' % node.name,
                )
            if node.anim_scale is not None:
                node.anim_scale.to_fcurves(
                    pose_bone,
                    armature_obj,
                    "scale",
                    'pose.bones["%s"].scale' % node.name,
                )
        ...
        if len(skinned_bone_ids):
            ...
            # Create animations
            armature_bones = [
                node_map[id] for id in node_map if node_map[id].name in armature.bones
            ]
        ...
        for geoset_id, geoset in enumerate(self.geosets):
            ...
            faces = [
                tuple(geoset.triangles[i : i + 3])
                for i in range(0, len(geoset.triangles), 3)
            ]  # Group triangles into tuples of 3
            ...
            if is_skinned:
                ...
                for bone_id in geoset_bones:
                    ...
                    if bone.object_id in skinned_bone_ids:
                        bone_groups[bone_id] = mesh_obj.vertex_groups.new(
                            name=bone.name
                        )
                ...
                if bone.geoset_anim_id is not None:
                    ...
                    if geoset_anim.alpha_anim is not None:
                        if bone_obj in bone_armature:
                            ...
                            geoset_anim.alpha_anim.to_fcurves(
                                bone_obj, bone_obj, "hide_render", "hide_render"
                            )

                mesh_obj = bpy.data.objects.new(
                    bone_name.replace("Bone_", "Geoset_"), mesh
                )
                context.collection.objects.link(mesh_obj)
        ...
        for node_type in self.objects:
            for node in self.objects[node_type]:
                ...
                if node_type in {
                    ...
                    "eventobject",
                    "collisionshape",
                }:  # There shouldn't be any bones left... but just in case
                    node_name = node.name
                    ...
                    if node_type == "collisionshape":
                        ...
                        obj.empty_display_type = {"Sphere": "SPHERE", "Box": "CUBE"}[
                            node.type
                        ]

                        if node.type == "Sphere":
                            obj.scale = global_matrix.to_3x3() @ Vector(
                    ...
                    bpy.ops.object.light_add(
                        type="POINT", radius=1.0, align="WORLD", location=pivot
                    )
                    ...
                    if node.speed_anim is not None:
                        node.speed_anim.to_fcurves(
                            psys, settings, "speed", "mdl_particle_sys.speed"
                        )
                    if node.variation_anim is not None:
                        node.variation_anim.to_fcurves(
                            psys, settings, "variation", "mdl_particle_sys.variation"
                        )
                    if node.emission_rate_anim is not None:
                        node.emission_rate_anim.to_fcurves(
                            psys,
                            settings,
                            "emission_rate",
                            "mdl_particle_sys.emission_rate",
                        )
                    if node.gravity_anim is not None:
                        node.gravity_anim.to_fcurves(
                            psys, settings, "gravity", "mdl_particle_sys.gravity"
                        )
                    if node.latitude_anim is not None:
                        node.latitude_anim.to_fcurves(
                            psys, settings, "latitude", "mdl_particle_sys.latitude"
                        )
                ...
                if node_type == "eventobject":
                    ...
                    if node.track is not None:
                        node.track.to_fcurves(
                            obj, obj, '["event_track"]', '["event_track"]'
                        )
                ...
                if node.anim_loc is not None:
                    node.anim_loc.to_fcurves(
                        obj, obj, "location", "location", global_matrix
                    )
                if node.anim_rot is not None:
                    node.anim_rot.to_fcurves(
                        obj, obj, "rotation_quaternion", "rotation_quaternion"
                    )
                if node.anim_scale is not None:
            ...
            for node in self.objects[node_type]:
                ...
                if node.parent_id is not None:
                    if objects[node.parent_id] in bone_armature:
                        ...
                        child.matrix_parent_inverse = armature.data.bones[
                            bone_name
        ...
        for sequence in scene.mdl_sequences:
            sequences.append(
                War3AnimationSequence(
                    sequence.name,
                    sequence.start * self.f2ms,
                    sequence.end * self.f2ms,
                    sequence.non_looping,
                    sequence.move_speed,
                )
            )
